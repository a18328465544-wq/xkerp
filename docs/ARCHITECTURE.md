...
高频查询仍保留 JSONB 作为业务文档，但为库存 SN、商品/状态、入库时间、单据号、流水时间和会话过期时间建立了表达式索引。`GET /api/inventory/items` 与 Open API 库存列表直接使用 PostgreSQL 分页和过滤，不再先把全部库存加载进 Node 进程。
...
2. **JSONB 集合表便于快速迭代，但查询能力有限**  
   当数据达到数十万级时，应为常用查询字段增加独立列或索引，例如 SN、商品 ID、客户 ID、单据日期、库存状态、手机号。
