...
| 能力 | 当前状态 | 当前前端策略 | 后续建议 |
| --- | --- | --- | --- |
| 独立库存详情 | 没有登录态下的单条库存接口 | 用 `/api/inventory/items` 精确 ID 兼容查询 | 增加 `GET /api/inventory/items/:id` |
...
`/api/inventory/items` 已支持服务端分页、keyword、brand、warehouseLocation、status、risk、库龄、利润率、activeOnly、includeSold、排序；`/api/inventory/summary` 提供按商品聚合的数量和成本摘要。
