...
test("finance transfers endpoint follows the existing state and mutation paths", async () => {
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (String(input).includes("/api/state")) return new Response(JSON.stringify({data: {accountTransfers: []}}), {status: 200, headers: {"Content-Type": "application/json"}});
  ...
  try {
    ...
    assert.equal(calls[0]?.url, "/api/state?mode=full");
