...
export function createFinanceLedgerColumns(onView: (item: FinanceLedgerItem) => void): ColumnDef<FinanceLedgerItem, unknown>[] {
  return [
    ...
    {id: "changeAmount", accessorKey: "changeAmount", header: "金额(元)", size: 130, enableSorting: false, cell: ({row}) => <span className={`font-mono font-bold ${row.original.changeAmount >= 0 ? "text-[var(--erp-color-success)]" : "text-[var(--erp-color-danger)]"}`}>{row.original.changeAmount >= 0 ? "+" : "−"}{formatLedgerCurrency(Math.abs(row.original.changeAmount))}</span>},
