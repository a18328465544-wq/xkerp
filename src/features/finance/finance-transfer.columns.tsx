...
export function createFinanceTransferColumns({canEdit, canDelete, onView, onEdit, onDelete}: {canEdit: boolean; canDelete: boolean; onView: (item: FinanceTransferItem) => void; onEdit: (item: FinanceTransferItem) => void; onDelete: (item: FinanceTransferItem) => void}): ColumnDef<FinanceTransferItem, unknown>[] {
  return [
    ...
    {id: "fee", accessorKey: "fee", header: "手续费", size: 120, cell: ({row}) => <span className="font-mono text-[var(--erp-color-warning)]">{formatCurrency(row.original.fee)}</span>},
    {id: "receivedAmount", accessorKey: "receivedAmount", header: "实际到账", size: 140, cell: ({row}) => <span className="font-mono font-bold text-[var(--erp-color-success)]">{formatCurrency(row.original.receivedAmount)}</span>},
