...
export function createFinanceIncomeColumns({canEdit, canDelete, onView, onEdit, onDelete}: {canEdit: boolean; canDelete: boolean; onView: (item: FinanceIncomeItem) => void; onEdit: (item: FinanceIncomeItem) => void; onDelete: (item: FinanceIncomeItem) => void}): ColumnDef<FinanceIncomeItem, unknown>[] {
  return [
    ...
    {id: "amount", accessorKey: "amount", header: "金额", size: 140, cell: ({row}) => <span className="erp-data-number font-semibold text-[var(--erp-color-income)]">+{formatCurrency(row.original.amount)}</span>},
    ...
    {id: "referenceNo", accessorKey: "referenceNo", header: "外部参考号", size: 150, cell: ({row}) => <span className="erp-data-number text-xs">{row.original.referenceNo || "—"}</span>},
