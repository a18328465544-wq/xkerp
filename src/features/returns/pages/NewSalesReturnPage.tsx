...
function SalesReturnForm({session, invoices, inventory, onAuthExpired, onSuccess}: {session: AuthSession; invoices: SalesInvoice[]; inventory: Array<{id: string; sn: string; salesInvoiceId?: string; status: string}>; onAuthExpired: () => void; onSuccess: () => void}) {
  ...
  const mutation = useMutation({mutationFn: () => returnsApi.createSales({...values, amount: Number(selectedItem?.sellPrice || values.amount), sourceInventoryId: selectedCard?.id || values.sourceInventoryId, productId: selectedItem?.productId || values.productId, productName: selectedItem?.productName || values.productName, sn: selectedItem?.sn || values.sn, partyName: selectedInvoice?.customerName || values.partyName, partyId: selectedInvoice?.customerId || values.partyId, contact: selectedInvoice?.contact || values.contact})});

  const setInvoice = (invoiceNo: string) => {
    const invoice = eligibleInvoices.find((item) => item.invoiceNo === invoiceNo);
    setValues((current) => ({...current, relatedDocNo: invoiceNo, sourceSalesItemIndex: -1, sourceInventoryId: "", productId: "", productName: "", sn: "", partyName: invoice?.customerName || "", partyId: invoice?.customerId || "", contact: invoice?.contact || "", amount: 0}));
  };
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedItem) return setError("请选择需要退货的商品明细");
    if (!selectedCard) return setError("该商品缺少可追溯的库存卡片，无法安全办理退货");
  ...
  return <ErpTransactionPageFrame className="max-w-[1300px]">
    ...
    <form className="flex flex-col gap-5" onSubmit={submit}>
      ...
      <ErpFormSection title="退货商品" description="服务端会再次校验商品是否属于该销售单，避免同名商品误退。"><div className="grid gap-4 md:grid-cols-2">
        <label className="text-sm font-semibold">商品明细<Select searchable searchPlaceholder="搜索商品名称或 SN" className="mt-2" value={values.sourceSalesItemIndex >= 0 ? String(values.sourceSalesItemIndex) : ""} options={(selectedInvoice?.items || []).map((item, index) => ({value: String(index), label: `${item.productName} · ${item.sn || "未记录 SN"} · ¥${item.sellPrice}`}))} onValueChange={setItem} placeholder={selectedInvoice ? "请选择退货商品" : "先选择销售单"} disabled={!selectedInvoice} aria-label="退货商品" /></label>
        <label className="text-sm font-semibold">原库存卡片<Input className="mt-2" value={selectedCard ? `${selectedCard.id} · ${selectedCard.sn || "无 SN"}` : "未匹配"} disabled /></label>
        <label className="text-sm font-semibold">原成交价<Input className="mt-2" value={selectedItem ? `¥${selectedItem.sellPrice.toLocaleString("zh-CN")}` : "—"} disabled /></label>
        <label className="text-sm font-semibold">库存处理<Select className="mt-2" value={values.inventoryAction} options={actionOptions} onValueChange={(value) => setValues((current) => ({...current, inventoryAction: value as SalesReturnFormValues["inventoryAction"]}))} aria-label="库存处理方式" /></label>
      </div></ErpFormSection>
