...
import {ErpDataTable, ErpEmptyState, ErpPageError, ErpPageHeader, ErpSettingsPageFrame} from "@/src/components/common";
...
function BackupContent({session, backups, loading, creating, onRefresh, onCreate, onDownload}: {session: AuthSession; backups: BackupRecord[]; loading: boolean; creating: boolean; onRefresh: () => void; onCreate: () => void; onDownload: () => void}) {
  ...
  return <><ErpPageHeader density="default" title="数据备份" subtitle={`仅老板可用 · 当前账号：${session.user.displayName}`} actions={<><Button variant="secondary" size="sm" onClick={onRefresh}><RefreshCw className="h-4 w-4" />刷新</Button><Button variant="secondary" size="sm" onClick={onDownload}><Download className="h-4 w-4" />下载业务数据</Button><Button variant="primary" size="sm" disabled={creating} onClick={onCreate}><Archive className="h-4 w-4" />{creating ? "备份中…" : "立即备份"}</Button></>} /><Card><CardContent className="flex items-start gap-3 p-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--erp-color-info-soft)] text-[var(--erp-color-primary)]"><ShieldCheck className="h-5 w-5" /></span><div><p className="font-semibold">备份不会修改业务数据</p><p className="mt-1 text-sm text-[var(--erp-color-text-secondary)]">手动备份会在服务端生成数据库快照；下载文件已按后端规则移除账号口令哈希。</p></div></CardContent></Card>{!loading && !backups.length ? <Card><ErpEmptyState title="暂无备份记录" description="点击“立即备份”创建第一份快照。" /></Card> : <ErpDataTable columns={columns} data={backups} getRowId={(row) => row.id} loading={loading} emptyTitle="暂无备份记录" emptyDescription="点击“立即备份”创建第一份快照。" />}</>;
}
