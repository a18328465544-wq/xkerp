...
export function createInventoryColumns({showCost, showProfit, onDetail}: {showCost: boolean; showProfit: boolean; onDetail: (item: InventoryListItem) => void}): ColumnDef<InventoryListItem, unknown>[] {
  ...
  columns.push({id: "estimatedSellPrice", header: "售价", accessorFn: (row) => row.inventoryStatus === "已售出" ? row.salesPrice : row.estimatedSellPrice, size: 110, enableSorting: false, cell: ({row}) => { const sold = row.original.inventoryStatus === "已售出"; return <div><p className="font-mono">{amount(sold ? row.original.salesPrice : row.original.estimatedSellPrice)}</p><p className="text-[11px] text-[var(--erp-color-text-muted)]">{sold ? "成交价" : "预计售价"}</p></div>; }});
  if (showProfit) columns.push({id: "estimatedProfit", header: "利润", accessorFn: (row) => row.inventoryStatus === "已售出" ? row.actualProfit : row.estimatedProfit, size: 110, enableSorting: false, cell: ({row}) => { const sold = row.original.inventoryStatus === "已售出"; return <div><ProfitDisplay value={sold ? row.original.actualProfit : row.original.estimatedProfit} /><p className="text-[11px] text-[var(--erp-color-text-muted)]">{sold ? "成交毛利" : "预计利润"}</p></div>; }});
  columns.push(
