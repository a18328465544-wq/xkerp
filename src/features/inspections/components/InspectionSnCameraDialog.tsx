...
export function InspectionSnCameraDialog({open, onOpenChange, onDetected}: {open: boolean; onOpenChange: (open: boolean) => void; onDetected: (code: string) => void}) {
      ...
      try {
        ...
        const scan = async () => {
          ...
          try {
            ...
            // A single frame can fail while the camera is focusing.
          }
    ...
    <Dialog.Portal>
      ...
      <Dialog.Viewport className="fixed inset-0 erp-modal-layer flex items-center justify-center p-4">
        <Dialog.Popup className="w-full max-w-xl rounded-[var(--erp-radius-xl)] border border-[var(--erp-color-border)] bg-[var(--erp-color-surface)] shadow-[var(--erp-shadow-popover)]">
          <div className="flex items-start justify-between gap-4 border-b border-[var(--erp-color-border)] px-5 py-4"><div><Dialog.Title className="flex items-center gap-2 text-base font-bold"><Camera className="h-4 w-4 text-[var(--erp-color-primary)]" />扫码录入实物 SN</Dialog.Title><Dialog.Description className="mt-1 text-xs text-[var(--erp-color-text-secondary)]">识别条形码或二维码；识别成功后只回填当前检测表单。</Dialog.Description></div><Dialog.Close render={<Button type="button" size="icon" variant="ghost" aria-label="关闭">×</Button>} /></div>
