...
export function createSalesListColumns({showProfit, canDelete, onDetail, onDelete}: {showProfit: boolean; canDelete: boolean; onDetail: (item: SalesListItem) => void; onDelete: (item: SalesListItem) => void}): ColumnDef<SalesListItem, unknown>[] {
  const columns: ColumnDef<SalesListItem, unknown>[] = [
    {accessorKey: "invoiceNo", header: "销售单号", size: 180, cell: ({row}) => <p className="erp-data-number font-semibold text-[var(--erp-color-primary)]">{row.original.invoiceNo}</p>},
    {accessorKey: "date", header: "日期", size: 110, cell: ({getValue}) => <span className="text-xs text-[var(--erp-color-text-secondary)]">{formatStoreDateTime(String(getValue() || ""))}</span>},
    ...
    {accessorKey: "totalCount", header: "数量", size: 80, cell: ({row}) => <span className="erp-data-number font-semibold">{row.original.totalCount} 件</span>},
    {accessorKey: "totalAmount", header: "销售金额", size: 120, cell: ({row}) => <span className="erp-data-number font-semibold">{formatCurrency(row.original.totalAmount)}</span>},
  ];
  if (showProfit) columns.push({accessorKey: "totalProfit", header: "销售利润", size: 115, cell: ({row}) => <span className="erp-data-number font-semibold text-[var(--erp-color-success)]">{row.original.totalProfit === undefined ? "—" : formatCurrency(row.original.totalProfit)}</span>});
  columns.push(
    ...
    {id: "actions", header: "操作", enableSorting: false, enableResizing: false, enableHiding: false, size: canDelete ? 150 : 82, cell: ({row}) => <div className="flex items-center gap-1" onClick={(event) => event.stopPropagation()}><Button type="button" size="sm" variant="ghost" onClick={() => onDetail(row.original)}><Eye className="h-3.5 w-3.5" />详情</Button>{canDelete && (row.original.outboundStatus === "已出库" ? <span className="inline-flex h-8 items-center gap-1 px-2 text-xs text-[var(--erp-color-text-muted)]" title="已出库销售单不能删除" aria-label={`${row.original.invoiceNo} 已出库，不能删除`}><Trash2 className="h-3.5 w-3.5" />不可删</span> : <Button type="button" size="sm" variant="ghost" className="text-[var(--erp-color-danger)] hover:text-[var(--erp-color-danger)]" title="删除销售单" aria-label={`删除${row.original.invoiceNo}`} onClick={() => onDelete(row.original)}><Trash2 className="h-3.5 w-3.5" />删除</Button>)}</div>},
  );
  return columns;
}
