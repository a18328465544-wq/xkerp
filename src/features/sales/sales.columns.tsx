...
export function createSalesListColumns({showProfit, onDetail}: {showProfit: boolean; onDetail: (item: SalesListItem) => void}): ColumnDef<SalesListItem, unknown>[] {
  ...
  columns.push(
    ...
    {id: "actions", header: "操作", enableSorting: false, enableResizing: false, enableHiding: false, size: 82, cell: ({row}) => <Button type="button" size="sm" variant="ghost" onClick={(event) => {event.stopPropagation(); onDetail(row.original);}}><Eye className="h-3.5 w-3.5" />摘要</Button>},
  );
  return columns;
}
