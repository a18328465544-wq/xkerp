...
export function SalesLineItemsTable({control, fields, selectedCandidates, pickerKeyword, pickerOptions, pickerLoading, pickerError, pickerDisabled, onPickerKeywordChange, onPickerFocus, onPickerRetry, onCandidateSelect, onCandidateClear, onAdd, onRemove}: {
  ...
  pickerOptions: (fieldId: string) => SalesInventoryCandidate[];
  ...
  return <Card data-erp-component="transaction-line-items"><CardContent className="p-3 sm:p-4">
    <div className="hidden grid-cols-[minmax(0,2.2fr)_5rem_9rem_8rem_minmax(0,1.6fr)_3rem] gap-0 border-b border-[var(--erp-color-border)] pb-2 text-left text-xs font-semibold text-[var(--erp-color-text-secondary)] sm:grid">
      <div className="px-2">商品 / 库存候选</div>
    ...
    <div className="space-y-3 sm:space-y-0">{fields.map((field, index) => {
      ...
      return <div key={field.id} className="relative grid min-w-0 grid-cols-2 gap-x-3 gap-y-3 rounded-[var(--erp-radius-lg)] border border-[var(--erp-color-border)] p-3 sm:grid-cols-[minmax(0,2.2fr)_5rem_9rem_8rem_minmax(0,1.6fr)_3rem] sm:gap-0 sm:rounded-none sm:border-x-0 sm:border-t-0 sm:px-2 sm:py-3 sm:last:border-0">
        <div className="col-span-2 min-w-0 pr-10 sm:col-span-1 sm:pr-0">
          ...
          {selected && <div className="mt-2 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 text-xs text-[var(--erp-color-text-muted)]"><span className="break-words">{selected.brand} {selected.model}</span><span aria-hidden="true">·</span><span>{selected.vram || "无显存信息"}</span><span aria-hidden="true">·</span><span>{selected.serialNumber ? `可检索 SN：${selected.serialNumber}` : "SN 将在出库阶段绑定"}</span></div>}
        </div>
          ...
          <Controller
            ...
            render={({field: input}) => (
              <Input
                ...
                onChange={(event) => input.onChange(Math.max(1, Number(event.target.value) || 1))}
                onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); document.querySelector<HTMLElement>(`[aria-label="选择销售商品候选"]`)?.focus(); } }}
        ...
        <div className="col-span-2 min-w-0 sm:col-span-1 sm:px-2">
          ...
          {selected ? <ErpStatusBadge label={selected.saleable ? "可销售候选（待出库绑定 SN）" : "不可销售"} tone={selected.saleable ? "success" : "danger"} /> : <span className="text-xs text-[var(--erp-color-text-muted)]">待选择</span>}
        </div>
        <div className="col-span-2 min-w-0 sm:col-span-1 sm:px-2">
    ...
    })}</div><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><Button type="button" variant="secondary" size="sm" onClick={onAdd}><Plus className="h-4 w-4" />增加商品</Button><p className="text-xs text-[var(--erp-color-text-muted)]">库存候选来自当前可售库存，重复候选会被表单拦截。</p></div>
  </CardContent></Card>;
}
