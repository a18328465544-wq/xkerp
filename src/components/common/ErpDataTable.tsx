    ...
    <div ref={scrollRef} className={cn("erp-scrollbar erp-horizontal-scroll overflow-x-auto", showMobileCards && "hidden sm:block", virtualized && "max-h-[min(48rem,68vh)] overflow-y-auto")}>
      <table className="w-full min-w-[1180px] border-collapse text-left text-sm" aria-label={ariaLabel} aria-busy={loading || fetching} aria-rowcount={total ?? undefined}>
        ...
        <tbody style={virtualized ? {height: `${rowVirtualizer.getTotalSize()}px`, position: "relative"} : undefined}>
          {visibleRows.map(({row, start}) => <tr key={row.id} style={virtualized ? {position: "absolute", top: 0, left: 0, width: "100%", transform: `translateY(${start ?? 0}px)`} : undefined} tabIndex={onRowClick ? 0 : undefined} className={cn("border-b border-[var(--erp-color-border)] last:border-0", onRowClick && "cursor-pointer hover:bg-[var(--erp-color-info-soft)] focus-visible:bg-[var(--erp-color-info-soft)]")} onClick={() => onRowClick?.(row.original)} onKeyDown={(event) => { if (onRowClick && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onRowClick(row.original); } }}>
