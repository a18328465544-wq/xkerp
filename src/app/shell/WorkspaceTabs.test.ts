...
test("workspace tab links delegate switch intent before route navigation", () => {
  ...
  assert.match(workspaceSource, /event\.preventDefault\(\);[\s\S]*setNavigationIntent\(current \? null : "switch"\);[\s\S]*activate\(item\.id\);[\s\S]*const targetPath = routeByTabRef\.current\[item\.id\] \|\| item\.path;[\s\S]*void navigate\(\{to: targetPath(?:, search: searchWithoutDetail\(\))?\}\)/);
