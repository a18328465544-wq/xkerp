...
export function createStoreActions(state: AppState, context: StoreActionContext = {}) {
  ...
  // Customer order pool ---------------------------------------------------
  // The order pool is deliberately a collaboration aggregate, not a second
  // sales/purchase ledger. It stores only the customer intent, current work
  // state, people involved, links to source documents and an auditable feed.
  const orderPoolStages = new Set<OrderPoolStage>([
    "待接单", "跟进中", "待客户", "待执行", "已完成", "暂停", "丢单", "取消", "售后中",
  ]);
  const orderPoolBlockers = new Set<OrderPoolBlocker>([
    "待报价", "待备货", "待收款", "待出库", "待估价", "待收货", "待检测", "待付款", "待入库", "待客户确认",
  ]);
  const orderPoolTypes = new Set(["销售", "回收", "置换"] as const);
  ...
  const orderPoolDocumentTypes = new Set(["quote", "sales", "purchase", "return", "inspection", "payment_in", "payment_out", "inventory", "aftersales"] as const);
  ...
  const assertOrderPoolRecord = (value: unknown, label: string): Record<string, unknown> => {
    if (!value || typeof value !== "object" || Array.isArray(value)) throw new ValidationError(`${label}格式无效`);
  ...
  const resolveOrderPoolCollaborators = (ids: unknown, ownerId?: string) => {
    ...
    const uniqueIds = Array.from(new Set(ids.map((value) => cleanOrderPoolText(value, "协作者账号", 100)).filter(Boolean)));
    const joinedAt = nowStamp();
    return uniqueIds.map((userId) => {
      const resolved = resolveOrderPoolUser(userId);
      return {userId: resolved?.userId || userId, displayName: resolved?.displayName || userId, joinedAt};
  ...
  const addCustomerOrderEvent = (order: CustomerOrder, input: {type: CustomerOrder["events"][number]["type"]; content: string; metadata?: Record<string, string>}) => {
    const event = {
      id: genId("ODE"),
      type: input.type,
  ...
  const assertOrderPoolBaseFields = (input: {orderType?: unknown; partyType?: unknown; mainStage?: unknown; blocker?: unknown; priority?: unknown}) => {
    if (!orderPoolTypes.has(input.orderType as "销售" | "回收" | "置换")) throw new ValidationError("订单类型无效");
    if (input.partyType !== undefined && !orderPoolPartyTypes.has(input.partyType as "customer" | "vendor" | "mixed")) throw new ValidationError("关联主体类型无效");
    if (input.mainStage !== undefined && !orderPoolStages.has(input.mainStage as OrderPoolStage)) throw new ValidationError("订单阶段无效");
    if (input.blocker !== undefined && input.blocker !== null && !orderPoolBlockers.has(input.blocker as OrderPoolBlocker)) throw new ValidationError("待办标签无效");
    if (input.priority !== undefined && !orderPoolPriorities.has(input.priority as "low" | "normal" | "high" | "urgent")) throw new ValidationError("优先级无效");
  };
  const createCustomerOrder = (input: OrderPoolCreateInput) => {
    ...
    const partyType = input.partyType || (orderType === "回收" ? "customer" : "customer");
    if (!orderPoolPartyTypes.has(partyType)) throw new ValidationError("关联主体类型无效");
    const now = nowStamp();
    const owner = resolveOrderPoolUser(input.ownerId, input.ownerName) || {userId: context.userId, displayName: getActiveActor()};
  ...
  const updateCustomerOrder = (id: string, patch: OrderPoolUpdateInput) => {
    ...
    assertOrderPoolBaseFields({orderType: existing.orderType, mainStage: patch.mainStage, blocker: patch.blocker, priority: patch.priority});
    ...
    const owner = patch.ownerId !== undefined || patch.ownerName !== undefined
      ? (patch.ownerId === null && patch.ownerName === null ? undefined : resolveOrderPoolUser(patch.ownerId || undefined, patch.ownerName || undefined))
    ...
    const nextBlocker = patch.blocker === null || nextStage === "已完成" || ["丢单", "取消"].includes(nextStage)
      ? undefined
      : patch.blocker === undefined ? existing.blocker : patch.blocker;
    ...
    if (nextStage !== beforeStage) addCustomerOrderEvent(next, {type: "stage_changed", content: `阶段由「${beforeStage}」调整为「${nextStage}」`, metadata: {from: beforeStage, to: nextStage}});
  ...
  const linkCustomerOrderDocument = (id: string, input: OrderPoolDocumentLinkInput) => {
    ...
    const existing = state.customerOrders.find((item) => item.id === id || item.orderNo === id);
    if (!existing) throw new NotFoundError(`协同订单不存在: ${id}`);
    if (!orderPoolDocumentTypes.has(input.type)) throw new ValidationError("关联单据类型无效");
    ...
    if (!next.linkedDocuments.some((item) => item.type === input.type && item.id === documentId)) {
      next.linkedDocuments = [{type: input.type, id: documentId, label: cleanOrderPoolText(input.label, "关联单据名称", 160) || undefined, linkedAt: nowStamp(), linkedBy: getActiveActor()}, ...next.linkedDocuments];
      addCustomerOrderEvent(next, {type: "link_added", content: `关联${input.type}单据 ${documentId}`, metadata: {type: input.type, id: documentId}});
      state.customerOrders = state.customerOrders.map((item) => item.id === existing.id ? next : item);
