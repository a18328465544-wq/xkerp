...
export function buildExport(state: AppState, dataset: string, options: ExportOptions): { filename: string; csv: string } {
  ...
  switch (dataset) {
    case "inventory": {
      ...
      return { filename: `库存明细_${stamp}.csv`, csv: toCsv(headers, rows) };
    }
    case "sales": {
      ...
      return { filename: `销售单_${stamp}.csv`, csv: toCsv(headers, rows) };
    }
    case "purchases": {
      ...
      return { filename: `进货单_${stamp}.csv`, csv: toCsv(headers, rows) };
    }
    case "ledger": {
      ...
      return { filename: `财务流水_${stamp}.csv`, csv: toCsv(headers, rows) };
    }
    case "customers": {
      ...
      return { filename: `客户档案_${stamp}.csv`, csv: toCsv(headers, rows) };
    }
  }
}
