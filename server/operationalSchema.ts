...
export const OPERATIONAL_PROJECTION_SCHEMA_VERSION = "operational-projections-v1";

/**
 * Additive query projections for the high-frequency JSONB documents.
 * JSONB stays authoritative; generated columns cannot drift because PostgreSQL
 * derives them on every insert/update and existing writers remain unchanged.
 */
export const OPERATIONAL_PROJECTION_SQL = `
  ...
  VALUES ('${OPERATIONAL_PROJECTION_SCHEMA_VERSION}')
  ON CONFLICT (version) DO NOTHING;
`;

export async function applyOperationalProjectionSchema(client: PoolClient) {
  await client.query(OPERATIONAL_PROJECTION_SQL);
}
